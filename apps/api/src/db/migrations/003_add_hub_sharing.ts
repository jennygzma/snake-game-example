...
export const up = (db: Database): void => {
  // Shared themes table (public, profile-scoped)
  db.exec(`
  ...
  // Shared variations table (public, profile-scoped)
  db.exec(`
  ...
  // Theme favorites junction table
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_theme_favorites_profile ON theme_favorites(profile_id);
    CREATE INDEX IF NOT EXISTS idx_theme_favorites_theme ON theme_favorites(shared_theme_id);
  `);

  // Variation favorites junction table
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_variation_favorites_profile ON variation_favorites(profile_id);
    CREATE INDEX IF NOT EXISTS idx_variation_favorites_variation ON variation_favorites(shared_variation_id);
  `);
};

export const down = (db: Database): void => {
  db.exec(`
    DROP INDEX IF EXISTS idx_variation_favorites_variation;
    DROP INDEX IF EXISTS idx_variation_favorites_profile;
    DROP TABLE IF EXISTS variation_favorites;

    DROP INDEX IF EXISTS idx_theme_favorites_theme;
    DROP INDEX IF EXISTS idx_theme_favorites_profile;
